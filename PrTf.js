...
$("figure").hover(
    ...
    $(this).find(".On").css({"transform":"scale(2)",
      "opacity" : "1",
      "transition-duration":"1s"});
    $(this).find(".readyProject").css({"opacity" : "1", "transition-duration":"1s"});
...
const dragAndDrop = () => {
  ...
  const dragEnter = function(event){ //пересечение границы блока
  	event.preventDefault()
  	this.classList.remove('hovered')
  }

  const dragLeave = function(){// покидает границы блока
  	this.classList.add('hovered')
  }

  const dragDrop = function(){// бросаем картинку
  	this.classList.add('hovered')
  	this.append(ideaIMG)
  }
...
function touchMove(event) {
  ...
  drag.style.top = `${touch.pageY - (wrapper.offsetTop*1.13) /*- (drag.offsetWidth / 2)*/}px`;
